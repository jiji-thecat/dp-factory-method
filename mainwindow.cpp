...
void MainWindow::button1Clicked(){
   ...
   createTableItems(r->getMeat().c_str(), r->getVegitables().c_str(), r->getSoupStock().c_str(), r->getBowl().c_str(), 0);
   addSoupImage(result);

   Cat *c = new Cat();
   result = c->cookSoup().c_str();
   createTableItems(c->getMeat().c_str(), c->getVegitables().c_str(), c->getSoupStock().c_str(), c->getBowl().c_str(), 1);
   addSoupImage(result);

   Bear *b = new Bear();
   result = b->cookSoup().c_str();
   createTableItems(b->getMeat().c_str(), b->getVegitables().c_str(), b->getSoupStock().c_str(), b->getBowl().c_str(), 2);
   addSoupImage(result);
}

void MainWindow::createTableItems(QString meat, QString vegitable, QString stock, QString bowl, int column){
    ...
    tableWidget->setItem(3, column, new QTableWidgetItem(bowl));
